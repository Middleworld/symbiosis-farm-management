...
### Example `.env` placeholder entries

Here's a safe template you can drop into your admin suite `.env` (replace each value before going live):

```ini
# farmOS connection
FARMOS_URL=https://farmos.middleworldfarms.org
FARMOS_USERNAME=your-farmos-username
FARMOS_PASSWORD=your-strong-password
FARMOS_OAUTH_CLIENT_ID=your-client-id
FARMOS_OAUTH_CLIENT_SECRET=your-client-secret
FARMOS_OAUTH_SCOPE=farm_manager
FARMOS_AUTO_SYNC=true
```

> Keep the real secrets in your production `.env`; commit only placeholder versions.
