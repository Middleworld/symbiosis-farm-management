...
class MWF_Reviews {
    ...
    public function enqueue_styles() {
        ...
        if (is_object($post) && isset($post->post_content)) {
            $has_shortcode = has_shortcode($post->post_content, 'mwf_reviews');
        }
        
        // Also check if we're on the home page (reviews often on homepage)
        if (!$has_shortcode && (is_front_page() || is_home())) {
            $has_shortcode = true;
        }
        ...
        if ($has_shortcode) {
            ...
            // Plugin CSS
            wp_enqueue_style('mwf-reviews', plugin_dir_url(__FILE__) . 'assets/css/style.css', array('slick-theme'), '1.0.2');
            ...
            // Plugin JS
            wp_enqueue_script('mwf-reviews-js', plugin_dir_url(__FILE__) . 'assets/js/carousel.js', array('jquery', 'slick-carousel-js'), '1.0.2', true);
    ...
    public function reviews_shortcode($atts) {
        ...
        if (($atts['source'] === 'all' || $atts['source'] === 'facebook') && !empty(get_option('mwf_facebook_page_url'))) {
            ...
            $fb_adapt = 'true'; // Force adapt to container width for mobile
            ...
            $output .= '<div class="mwf-facebook-embed" style="margin-top: 20px; width: 100%; max-width: 100%;">';
