...
/**
 * Load environment variables from a root-level .env file if present.
 */
...
/**
 * Database configuration for farmOS
 */
$databases['default']['default'] = [
  'database' => getenv('FARMOS_DB_DATABASE') ?: 'soilsync-user_',
  'username' => getenv('FARMOS_DB_USERNAME') ?: 'soilsync_db',
  'password' => getenv('FARMOS_DB_PASSWORD') ?: 'LQhm^pZs?r9hqq13',
  'host' => getenv('FARMOS_DB_HOST') ?: 'localhost',
  'port' => getenv('FARMOS_DB_PORT') ?: '3306',
  'driver' => getenv('FARMOS_DB_DRIVER') ?: 'mysql',
  'prefix' => getenv('FARMOS_DB_PREFIX') ?: '',
  'collation' => getenv('FARMOS_DB_COLLATION') ?: 'utf8mb4_general_ci',
];
 ...
 * @endcode
 */
$settings['hash_salt'] = '6a5cbde6a23f427c59ebeb0b2698f3fc777c430ce90bed5f7d27e34e22b4fc2f';
 ...
 */
$settings['cookie_domain'] = '.soilsync.shop';
...
/**
 * Database settings for farmOS.
 */
$databases['default']['default'] = [
  'database' => 'soilsync-user_',
  'username' => 'soilsync_db',
  'password' => 'LQhm^pZs?r9hqq13',
  'prefix' => '',
  'host' => 'localhost',
  'port' => '3306',
...
# }
$base_url = "https://farmos.soilsync.shop";
$settings["reverse_proxy"] = TRUE;
...
/**
 * Allow iframe embedding from Laravel admin
 * For unified multi-environment admin interface
 * 
 * CRITICAL: Must allow same-origin iframes (*.soilsync.shop domain)
 */
// Always allow iframe embedding from same domain - no referer check needed
header('Content-Security-Policy: frame-ancestors https://admin.soilsync.shop https://*.soilsync.shop');
// Remove X-Frame-Options entirely (deprecated, conflicts with CSP)
header_remove('X-Frame-Options');

/**
 * Inject CSS/JS to hide farmOS UI elements when embedded in iframe
 */
if (isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'], 'admin.soilsync.shop') !== false) {
  // Add custom CSS to hide sidebar and other UI elements in iframe mode
  $config['system.theme.global']['css'] = [
    'farmOS_iframe_mode' => "
      /* Hide farmOS sidebar when embedded */
      ...
      /* Remove top padding from toolbar */
      body.toolbar-fixed {
...
/**
 * Detect iframe embed mode and inject CSS to hide UI
 */
if (isset($_GET['iframe_embed']) && $_GET['iframe_embed'] == '1') {
  // Set a custom library to inject CSS
  $config['system.performance']['css']['preprocess'] = FALSE;
}

/**
 * Share session cookies across all soilsync.shop subdomains  
 * This allows admin.soilsync.shop to embed farmos.soilsync.shop iframes
 */
$cookie_domain = ".soilsync.shop";
