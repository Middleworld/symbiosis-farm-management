...
(function(Drupal,drupalSettings){Drupal.behaviors.farm_map={attach:function(context,settings){context.querySelectorAll('[data-map-instantiator="farm_map"]').forEach(function(element){if(element.getAttribute('processed'))return;element.setAttribute('processed',true);element.setAttribute('tabIndex',0);const drupalSettingsKey=element.getAttribute('id');const mapInstanceOptions={};Drupal.behaviors.farm_map.createMapInstance(context,element,drupalSettingsKey,mapInstanceOptions);});},createMapInstance:function(context,element,drupalSettingsKey,mapInstanceOptions){let units='metric';if(!!drupalSettings.farm_map.units)units=drupalSettings.farm_map.units;const defaultOptions={units,interactions:{onFocusOnly:true}};const mapOptions={...defaultOptions,...drupalSettings.farm_map[drupalSettingsKey].instance,...mapInstanceOptions};const instance=farmOS.map.create(element,mapOptions);instance.farmMapSettings=drupalSettings.farm_map[drupalSettingsKey]||{};context.querySelectorAll('.ol-popup-closer').forEach(function(element){element.onClick=function(element){element.focus();};});new ResizeObserver(()=>instance.map.updateSize()).observe(element);return instance;}};}(Drupal,drupalSettings));;
(function(){farmOS.map.behaviors.sidePanel=farmOS.map.namedBehaviors.sidePanel;farmOS.map.behaviors.layerSwitcherInSidePanel=farmOS.map.namedBehaviors.layerSwitcherInSidePanel;}());;
(function(){farmOS.map.behaviors.popup={attach:function(instance){const featureName=function(feature){const name=feature.get('name');const url=feature.get('url');if(name===undefined||url===undefined)return name;return `<a href="${url}">${name}</a>`;};instance.popup=instance.addPopup(function(event){var content='';var clickedFeatures=instance.map.getFeaturesAtPixel(event.pixel);const sortValue=(feature)=>typeof feature.getGeometry().getArea==='function'?feature.getGeometry().getArea():0;clickedFeatures.sort((a,b)=>sortValue(a)-sortValue(b));var feature=clickedFeatures[0];if(feature){var features=feature.get('features');if(features!==undefined){var names=[];features.forEach(function(item){const name=featureName(item);if(name!==undefined)names.push(name);});if(names.length!==0)feature.set('description','<ul><li>'+names.join('</li><li>')+'</li></ul>');feature.set('name',names.length+' item(s):');}var name=featureName(feature)||'';if(name!==''){var description=feature.get('description')||'';var measurement=instance.measureGeometry(feature.getGeometry(),instance.units);content='<h4 class="ol-popup-name">'+name+'</h4><div class="ol-popup-measurement"><small>'+measurement+'</small></div><div class="ol-popup-description">'+description+'</div>';}}return content;});},weight:-100};}());;
(function(drupalSettings){farmOS.map.behaviors.asset_type_layers={attach:function(instance){if(instance.farmMapSettings.asset_type_layers!==undefined){var layers=instance.farmMapSettings.asset_type_layers;Object.values(layers).reverse().forEach((layer)=>{if(!layer.is_group)return;var opts={title:layer.label};if(!!layer.group)opts.group=layer.group;instance.addLayer('group',opts);});Object.values(layers).reverse().forEach((layer)=>{if(!!layer.is_group)return;let geomType='full';let layerType='geojson';if(!!layer.cluster&&layer.cluster){geomType='centroid';layerType='cluster';}const assetType=layer.asset_type||'all';const url=new URL('assets/geojson/'+geomType+'/'+assetType,window.location.origin+drupalSettings.path.baseUrl);const filters=layer.filters||{};Object.entries(filters).forEach(([key,value])=>{if(Array.isArray(value)){for(let i=0;i<value.length;i++)url.searchParams.append(key+'[]',value[i]);}else url.searchParams.append(key,value);});var opts={title:layer.label,url,color:layer.color};if(!!layer.group)opts.group=layer.group;var newLayer=instance.addLayer(layerType,opts);if(layerType!=='cluster'&&layer.zoom!==undefined&&layer.zoom){var source=newLayer.getSource();source.on('change',function(){instance.zoomToVectors();});}});}instance.popup.on('farmOS-map.popup',function(event){var link=event.target.element.querySelector('.ol-popup-name a');if(link){var assetLink=link.getAttribute('href');var description=event.target.element.querySelector('.ol-popup-description');var loading=document.createTextNode('Loading asset details...');description.appendChild(loading);var frame=document.createElement('iframe');frame.setAttribute('src',assetLink+'/map-popup');frame.onload=function(){description.removeChild(loading);instance.popup.panIntoView();};description.appendChild(frame);}});}};}(drupalSettings));;
